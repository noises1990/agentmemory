...
async function main() {
	...
	}).then((res) => reportCaptureResponse("stop:summarize", `${REST_URL}/agentmemory/summarize`, res), (err) => reportCaptureFailure("stop:summarize", `${REST_URL}/agentmemory/summarize`, err));
	fetch(`${REST_URL}/agentmemory/session/end`, {
		method: "POST",
		headers: authHeaders(),
		body: JSON.stringify({ sessionId }),
		signal: AbortSignal.timeout(5e3)
	}).then((res) => reportCaptureResponse("stop:session/end", `${REST_URL}/agentmemory/session/end`, res), (err) => reportCaptureFailure("stop:session/end", `${REST_URL}/agentmemory/session/end`, err));
