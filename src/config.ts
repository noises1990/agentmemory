...
export function loadConfig(): AgentMemoryConfig {
  ...
  // Port group: REST is the anchor; streams derives from it unless explicitly
  // overridden. The iii engine worker bus stays on the daemon's advertised
  // default 49134 unless III_ENGINE_URL/III_ENGINE_PORT is set by an operator.
  const restPort = parseInt(env["III_REST_PORT"] || "18111", 10) || 18111;
  const streamsPort =
  ...
  const engineUrl =
    env["III_ENGINE_URL"] ||
    `ws://localhost:${
      parseInt(env["III_ENGINE_PORT"] || "", 10) || 49134
    }`;
